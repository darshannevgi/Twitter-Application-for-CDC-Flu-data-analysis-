...
ui <- pageWithSidebar(
  ...
  sidebarPanel(
    selectInput('xcol', 'CDC vs Twitter Map', c("keywordall" = "keywordall",
                                                "keywordflu" = "keywordflu"))
  ),     # create a sidebar which can include actionButtons, drop-downs, etc.
  mainPanel(imageOutput(outputId = "imageTwitter",width = "100%"),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(), imageOutput(outputId = "imageCDC"),width = "100%")       # create a main panel to represent your charts, texts etc.
)
server <- function(input, output) {
  ...
  output$imageTwitter <- renderImage({
    filename1 <- normalizePath(file.path(dirname(rstudioapi::getSourceEditorContext()$path),
                                        paste(input$xcol, 'twitter.jpg', sep='')))
    # Return a list containing the filename and alt text
    list(src = filename1)
